...
/*
    ...
    Converting via a float timestamp would lose sub-second precision for
    values far from the epoch. Going through whole seconds keeps integer
    arithmetic throughout, and setting the microsecond field afterwards
    preserves the instant since UTC offsets only change on whole seconds.
*/
static PyObject *
