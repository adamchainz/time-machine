...
# that it targets, in varied formattings. The generated code only needs to
# parse, not run, so undefined names are fine.
...
@st.composite
def calls(draw, callee: str, arglists) -> str:
...
@st.composite
def method_statements(draw) -> str:
...
def test_migrate_contents_properties(source):
