...
* In class decorators on pytest-style test classes: ``freeze_time(...)`` -> ``pytest.mark.time_machine(...)``, the marker from time-machine’s :doc:`pytest plugin <pytest_plugin>`, with the same argument handling.
  ...
  Otherwise, it’s treated as pytest-style when it defines at least one ``test``-prefixed method, no ``__init__()`` or ``__new__()``, and either shows something unittest does not support: a test method taking fixture arguments, or an xunit-style hook like ``setup_method()``; or its name starts with ``Test``, pytest’s default for class collection, and all its base classes are ``object`` or classes defined in the same module that pass the same checks.
