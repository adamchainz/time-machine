...
* The ``pytest.mark.freeze_time`` marker from pytest-freezegun or pytest-freezer: ``@pytest.mark.freeze_time(...)`` -> ``@pytest.mark.time_machine(...)``, the marker from time-machine’s :doc:`pytest plugin <pytest_plugin>`, with the same argument handling as for ``freeze_time()`` calls.
