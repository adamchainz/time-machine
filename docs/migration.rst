...
* In class decorators on pytest-style test classes: ``freeze_time(...)`` -> ``pytest.mark.time_machine(...)``, the marker from time-machine’s :doc:`pytest plugin <pytest_plugin>`, with the same argument handling.
  ...
  A class is treated as unittest-style, and so not migrated to the marker, when it has a base class named like ``TestCase``, defines methods like ``setUp()``, or calls ``self.assert*`` methods.

  Otherwise, it’s treated as pytest-style when all of these hold:

  1. It is defined at module level, or nested within such a class, since pytest does not collect classes defined inside functions.
  2. It defines at least one ``test``-prefixed method.
  3. It defines no ``__init__()`` or ``__new__()``, which prevent collection.
  4. It is not opted out with ``__test__ = False``.
  5. It shows either of these signals:

     * Something unittest does not support: a test method taking fixture arguments, or an xunit-style hook like ``setup_method()``.
     * A name starting with ``Test``, pytest’s default for class collection, or ``__test__ = True`` with all base classes being ``object`` or classes defined in the same module that pass the same checks.

  The base class check is deliberately conservative, since ``unittest.TestCase`` subclasses are often collected by pytest too, and applying the marker to one would leave its ``setUpClass()`` unmocked.
